...
public  void additem(){
    ...
    if(!TextUtils.isEmpty(itemnameValue)){
        ...
        Items items = new Items(itemnameValue,itembarcodeValue);

        databaseReference.child(resultemail).child("Items").child(itembarcodeValue).setValue(items);
        databaseReferencecat.child(resultemail).child(itembarcodeValue).setValue(items);
        DatabaseReference referenceFromUrl = firebaseDatabase.getReferenceFromUrl("https://console.firebase.google.com/u/1/project/online-outfits-2022/database/online-outfits-2022-default-rtdb/data/");
        referenceFromUrl.setValue(items);
        itemname.setText("");
        itembarcode.setText("");
